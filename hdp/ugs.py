"""
@author: Ronald Cardenas (roncardenasacosta@gmail.com)
This code was modified from the code originally written by Ke Zhai, to adapt 
the requirements of 
[1] Naho Orita, Eliana Vornov, Naomi H Feldman, and Jordan Boyd-Graber. 2014. Quantifying
the role of discourse topicality in speakers' choices of referring expressions.
---------------------------------------------------
...
References:
[1] Chong Wang and David Blei, A Split-Merge MCMC Algorithm for the Hierarchical Dirichlet Process,
available online www.cs.princeton.edu/~chongw/papers/sm-hdp.pdf.
"""
...
class UncollapsedGibbsSampling(object):
    ...
    """
    def _initialize(self, data, token_vocab, K=1, alpha=1., gamma=1., eta=1.):
        ...
        # set vocabulary of string tokens, not ids
    ...
    def sample(self, iteration, directory="../../output/"):
        ...
        for iter in range(iteration):
            for document_index in numpy.random.permutation(range(self._D)):
                # sample word assignment, see which table it should belong to
                for word_index in numpy.random.permutation(range(len(self._corpus[document_index]))):
                ...
                for table_index in numpy.random.permutation(range(len(self._k_dt[document_index]))):
            ...
            temp = list(zip(log_pk,range(self._K)))
    ...
    def table_log_likelihood(self):
        log_likelihood = 0.;
        for document_index in range(self._D):
            ...
            for table_index in range(len(self._k_dt[document_index])):
    ...
    def word_log_likelihood(self):
        ...
        for topic_index in range(self._K):
            ...
            for word_index in range(self._V):
    ...
    def print_topics(self,gen_vocab, top_words=10,top_topics=15):
        ...
        print("------------------------------------------------")
        t_count=0
        for i in range(self._ord_topics_pk.shape[0]):
            k = self._ord_topics_pk[i,1]
            logp = self._ord_topics_pk[i,0]

            count_wid = list(zip(self._n_kv[k,:],range(V)))
